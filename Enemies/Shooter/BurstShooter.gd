...
export var engage_distance:int = 100
export var close_aim_chance:float = 0.5
export var far_aim_chance:float = 0.4
export var far_adjust_chance:float = 0.4
...
func decide_tactic():
	...
	reactionTimer.start()
	if get_distance_to_player() < engage_distance:
		var n = randf()
		if n < close_aim_chance:
			return State.AIM
		else:
		...
		if n < far_aim_chance:
			return State.AIM
		if n < far_aim_chance + far_adjust_chance:
			return State.ADJUST_WALK
		else:
